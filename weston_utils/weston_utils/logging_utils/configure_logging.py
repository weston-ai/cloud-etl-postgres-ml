...
import logging
from logging import Logger  # necessary for the return type annotation to be stated
import os
...
def configure_logging(
        ...
        log_file=None,
        level=logging.INFO
) -> Logger:   # return type annotation = Logger object
    """
    ...
    # Make sure that the log_file has ".log" extension; append ".log" it if missing
    ...
    os.makedirs(log_path.parent, exist_ok=True)   # log_path.parent points at log_dir, which is one level above log_file
    ...
    logger = logging.getLogger()   # Set up the root logger

    logger.info(f"Logging configured at '{os.path.abspath(log_path)}'")   # states the absolute (i.e. full) path
